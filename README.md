...
```
1. Manager creates outbound order
         ↓
...
├── api-gateway/
├── auth-service/         ✅ done
├── inventory-service/    ← replaces events-service
├── orders-service/       ← replaces booking-service
├── payment-service/
...
| Week | Focus |
|---|---|
| 1 | Redesign entities, migrations, update shared DTOs |
...
## What Makes This Impressive on a CV

- Unique domain — WMS is rare in portfolios, used in real enterprise systems
- Real microservices communication pattern
- Solved stock reservation race condition with Redis distributed locks
- CQRS pattern in Orders Service
- BullMQ for reliable email queue
